...
local QData = { range = myHero:GetSpellData(_Q).range, delay = 0.25, speed = 2000, width = 80 }
local WActive = false
...
local target = GetCurrentTarget()
	if Mix:Mode() == "Combo" then
		if ValidTarget(target, 1100) and Sad.Combo.Q:Value() and Ready(_Q) then
			local Prediction = GetPrediction(target, QData)
			if Prediction and Prediction.hitChance >= (Sad.Prediction.Q:Value()/100) and not Prediction:mCollision(1) then
